...
if __name__ == '__main__':
    ...
    # 打开保存结果的文件
    res_file = open("./result/test/diabetes.txt", "w", encoding='utf-8')
    # 创建空列表，保存预测结果
    res = []
    # 交叉验证开始
    tprs = []
    ...
    # 提取序列片段（阳性+阴性）
    # 打开阴阳数据集文件
    f_r = open("./dataset/Diabetes_Acetylation_protein/Diabetes_Acetylation_Pos_Neg.txt", "r", encoding='utf-8')
    # 正确打开文件后，读取文件内容
    ...
    for i in range(len(index_value)):
        ...
        res_file.write(data[0] + "\t" + data[3] + "\t" + str(index_value[i][1]) + "\t" + data[1] + "\n")
        res_file.flush()
